...
var TakeProfitSharwaFinance = class {
  constructor(provider, signer) {
    ...
    this.signer = signer;
    this.take_profit = new import_ethers.Contract(config_default.take_profit_address, abi_take_profit_default, signer);
    this.positions_manager = new import_ethers.Contract(config_default.positions_manager_address, abi_positions_manager_default, signer);
    this.operational_treasury = new import_ethers.Contract(config_default.operational_treasury_address, abi_operational_treasury_default, signer);
    this.iface = new import_ethers.Interface(abi_take_profit_default);
  }
  ...
  disableAutoExecutionForOption(tokenId) {
    return __async(this, null, function* () {
      yield this.positions_manager.approve(import_ethers.ZeroAddress, tokenId);
    });
  }
  ...
  filterOptionsWithAutoExecutionEnabled(user, arrAcivaOptions) {
    return __async(this, null, function* () {
      ...
      const DisableApprovalFilter = this.positions_manager.filters.Approval(user, import_ethers.ZeroAddress, null);
  ...
  getExecuteTakeProfits(user) {
    return __async(this, null, function* () {
      ...
      for (const i in arrLogsTakeProfitExecuted) {
        ...
        if (log instanceof import_ethers.EventLog) {
          const arg = log.args;
  ...
  _isApprovalForAllData(array) {
    ...
    array.forEach((item) => {
      if (item instanceof import_ethers.EventLog) {
        map.set(item.blockNumber, item.args.approved);
      } else if (item instanceof import_ethers.Log) {
